...
지원 언론사 펼침 목록은 Backend 공개 상태를 기준으로 `지원 중`, `일시 지원 중단`, `현재 미지원`을 문구로 구분한다. 색상만으로 상태를 전달하지 않는다. `현재 미지원`은 서비스가 시험·보강 대상으로 관리하는 언론사에 한정하고 국내 모든 언론사의 완전한 목록처럼 표현하지 않는다.

추출 실패의 내부 원인, 보안 차단 상세와 운영 로그는 노출하지 않는다. Figma에 펼침 목록의 Layout과 상태 표현이 없으면 임의로 디자인하지 않고 `[지원 언론사 펼침 목록의 Figma 디자인과 인터랙션 정보가 필요합니다.]`라고 요청한다.
